...
'rtype represents the type of report. 1 = p, 2 = s, 3 = u
Dim rtype As Integer

output = MsgBox("Would you like to generate a unified report for both Purchases and Sales?", vbYesNoCancel, "Unified Report for Purchases?")
If output = vbNo Then
    output = MsgBox("Would you like to generate a report for Purchases? (Yes for Purchases and No for Sales)", vbYesNoCancel, "Report for Purchases?")
    If output = vbNo Then
        rtype = 2
...
Dim acc As String
acc = InputBox("For what account do you wish to create a credit report for?", "Input Account Name", "ITOCHU")
acc = UCase(acc)
Dim icredit As Long
icredit = 0
Dim ucredit As Long
...
datedistance = 0
If rtype = 1 Then
    ws.Range("G8").Interior.ColorIndex = 38
...
End If
If rtype = 2 Then
    ws.Range("G8").Interior.ColorIndex = 456
    'Looping through Sales and adding relevant transactions to Credit report worksheet
    Do While data.Cells(i, 6).Value <> ""
        If data.Range("B" & row) = "SALES" Then
            If data.Range("F" & row) = acc Then
                datedistance = datediff("d", today, data.Range("U" & row))
                If datedistance >= 0 Then
                    ws.Range("A" & crow) = Format(data.Range("I" & row), "dd/mmm/yyyy")
...
End If
If rtype = 3 Then
    'resorting DATA by Account Name, Due Date
    With data.Sort
        .SortFields.Clear
        .SortFields.Add Key:=Range("F1"), Order:=xlAscending
        .SortFields.Add Key:=Range("U1"), Order:=xlAscending
        ...
        .Apply
    End With
    'formatting and column translation
    ws.Range("G8") = "AMT (PURCHASES)"
    ws.Range("H:H").EntireColumn.Insert
    ws.Range("H8") = "AMT (SALES)"
    ws.Range("G8").Interior.ColorIndex = 38
    ws.Range("H8").Interior.ColorIndex = 45
    'Looping through both Purchases and Sales and adding relevant transactions to Credit report worksheet
    Do While data.Cells(i, 6).Value <> ""
        If data.Range("B" & row) = "SALES" Or data.Range("B" & row) = "PURCHASES" Then
            If data.Range("F" & row) = acc Then
                datedistance = datediff("d", today, data.Range("U" & row))
                If datedistance >= 0 Then
                    ws.Range("A" & crow) = Format(data.Range("I" & row), "dd/mmm/yyyy")
                    ...
                    ElseIf data.Range("B" & row) = "SALES" Then
                        ws.Range("H" & crow) = data.Range("AJ" & row)
                        
                    End If
                    ws.Range("I" & crow) = "=G5-(SUM(G9:G" & crow & ")" & "-SUM(H9:H" & crow & "))"
                    ws.Range("J" & crow) = Format(data.Range("U" & row), "dd/mmm/yyyy")
...
If rtype = 3 Then ws.Range("A5") = "=SUM(G9:G500)-SUM(H9:H500)"
ws.Range("D5") = "=G5-A5"
...
If rtype = 1 Then
    ws.Range("D1") = "(PURCHASES REPORT)"
ElseIf rtype = 2 Then
    ws.Range("D1") = "(SALES REPORT)"
Else
    ws.Range("D1") = "(UNIFIED REPORT)"
End If
